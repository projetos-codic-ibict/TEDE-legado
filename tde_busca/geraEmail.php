...
<table width="100%" height="100%"  border="0" align="left" cellpadding="0" cellspacing="0">
  ...
  $col1 = array($txEnvioDeEmail);
  $dadosTabela = array($col1);
  $cfgCelulas = array(array("class =\"loginBusca\" align=\"center\""));
  $tabelaTitulo = gera_tabela($lin, $col, $cfgTabela, $cfgCelulas, $dadosTabela);

  $lin = 1;
  $col = 1;
  $cfgTabela = "width=\"80%\" align=\"center\" class=\"loginTable\"";
  $col1 = array("<a href=\"pastaTrabalhos.php\">".$txRetornarParaPasta." &raquo;</a>");
  $dadosTabela = array($col1);
  $cfgCelulas = array(array("align=\"center\" class=\"linkpreto\""));
  $tabelaCabecalho = gera_tabela($lin, $col, $cfgTabela, $cfgCelulas, $dadosTabela);

  //Campos do formulario
  $campoDestino = "<input type=\"text\" name=\"emailDestino\" value='' size=60 class=\"inputBusca\">";
  $campoResposta = "<input type=\"text\" name=\"emailResposta\" value='' size=60 class=\"inputBusca\">";
  $campoAssunto = "<input type=\"text\" name=\"emailAssunto\" value='' size=60 class=\"inputBusca\">";
  $campoTexto = "<textarea rows=\"10\" cols=\"60\" name=\"emailTexto\" wrap=\"soft\" class=\"inputBusca\"></textarea>";

  //Verifica se pasta existe
  if (session_is_registered('pasta'))
  {
   //Recebe array com tabelas HTML com os resultados detalhados
   $tabelaDados = gera_tabela_dados($pasta, 0, sizeof($pasta), array(), array(), array_values($pasta),0);
   //Texto complementar da msg composto dos dados da pasta
   $campoContTexto = '';
   for ($i=0;$i<sizeof($pasta);$i++)
   {
    //Preenche com as tabelas HTML dos registros
    $campoContTexto .= '<br />'.$tabelaDados[$i];
   }
  }
  else              //Pasta nao existe
  {
   //Tabela com informacao de pasta vazia
   $lin = 1;
   $col = 1;
   $cfgTabela = "width=\"80%\" align=\"center\" class=\"loginTable\"";
   $col1 = array($txSemDadosParaEnviar);
   $dadosTabela = array($col1);
   $cfgCelulas = array(array("class =\"subtitulo\" align=\"center\""));
   //Texto complementar a msg
   $campoContTexto = gera_tabela($lin, $col, $cfgTabela, $cfgCelulas, $dadosTabela);
  }

  //Botoes para o formulario
  $botoes = "<input name=\"Submit\" type=\"submit\" class=\"botao\" value=\"".$txEnviar."\">&nbsp;&nbsp;&nbsp;<input name=\"Reset\" type=\"reset\" class=\"botao\" value=\"".$txLimpar."\">";

  //Tabela com formulario
  $lin = 6;
  $col = 2;
  $cfgTabela = "width=\"80%\" align=\"center\" class=\"loginTable\"";
  $col1 = array($txParaEMail,$txDeEMail,$txAssunto,$txTexto,"&nbsp;",$txConteudoDaPasta);
  $col2 = array($campoDestino, $campoResposta, $campoAssunto, $campoTexto, $botoes, $campoContTexto);
  $dadosTabela = array($col1,$col2);
  $cfgCelulas = array(array("width=\"100\" class =\"campo\" align=\"left\" valign=\"top\"","class = \"texto\" align = \"left\" valign=\"top\""));
  $tabelaForm = gera_tabela($lin, $col, $cfgTabela, $cfgCelulas, $dadosTabela);

  //Construcao da pagina
  //Cabecalho
  $pagina = "$tabelaTitulo <br />";
  $pagina .= "$tabelaCabecalho <br />";
  $pagina .= "<form action=\"processaEmail.php\" method=\"GET\">\n";
  $pagina .= $tabelaForm;
  $pagina .= "</form>";
  ...
  //Monta a pagina
  //ob_start("monta");
  //echo monta_pagina("Pasta de Trabalhos - Envio por e-mail", $pagina);
