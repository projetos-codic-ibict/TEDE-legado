...
function normalizaNomeDiretorio($string){
    ...
    //Comentar essa linha pode solucionar alguns problemas de acentuação
...
function montaCitacaoDocumento($pfNomeAutor,$ttTitulo,$tsDataDefesa_ano, $pageCount,$grauTipo,
	...
	if (!(($pageCount==0) or ($pfNomeAutor=="") or ($prArea=="") or ($inNome=="") 
		...
		$citacao=citacaoNomeABNT($pfNomeAutor, true).". ".trim(htmlspecialchars($ttTitulo, ENT_QUOTES)).". ".
			...
			//$tsDataDefesa_ano .". ". $pageCount." f. ". $grauTipo ." (". $grau . trim($prArea). ") - " .
	...
	while ($SEL_ts_RES!="") {
		...
		*/
		...
		if ($tsGrau=="Doutor" or $tsGrau=="Doctor") {
			...
			//$type="<dcvalue element=\"type\" language=\"por\">Tese</dcvalue>";
		}elseif($tsGrau=="Mestre" or $tsGrau=="Master"){
			$grauTipo="Dissertação";
			//$grauTipo = utf8_decode('Dissertação');
			$type="<dcvalue element=\"type\" language=\"por\">". utf8_decode($grauTipo)."</dcvalue>";
			//$type="<dcvalue element=\"type\" language=\"por\">Dissertação</dcvalue>";
		}
		...
		if ($organizar=="PG"){
			...
			//$escreve2=fwrite($fp2,"\xEF\xBB\xBF".utf8_decode($arquivoxmlutf8)); 
			fclose($fp2);
			...
			//$fp2=fopen("/home/tede/public_html/tde_admin/arquivos_exportacao/dublin_core.xml",a);
			//$escreve2=fwrite($fp2,"\xEF\xBB\xBF".utf8_decode($arquivoxmlutf8)); 
			$escreve2=fwrite($fp2,$arquivoxmlutf8); 
			fclose($fp2);
