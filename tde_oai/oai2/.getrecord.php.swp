...
$versao_protocolo="1.0.7 (13-06-2006)";
...
class TAGXML 
{
	...
	* @return object TEDE_MTDBR
	...
	function tag($tag,$value,$atributos=null,$unescaped = false)
	{
		...
		if ($this->_xml_prefix!=null) $tag=$this->_xml_prefix.":".$tag;
		if ($unescaped){
	...
	$metadata = preg_replace('/[^\x09\x0A\x0D\x20-\x7F\xC0-\xFF]/', '', $metadata);
...
class TEDE_MTDBR extends TAGXML
{
	...
	* @return object TEDE_MTDBR
	...
	function getControleTipo($id)
	{
		...
		return $this->tag_obrigatorio('Tipo',"Tese ou Dissertação Eletrônica");
	...
	function getBibliotecaDigitalURLDocumento($id)
	{
		...
		// Patch de segurança
		// $r.=$this->tag_obrigatorio('URLDocumento', str_replace("http://http://","http://",$this->trata("http://".$this->bbURL.str_replace("..","",$row['arURL']))),$a);
	...
	function makeURL($cod){
		return str_replace("http://http://","http://",$this->trata("http://".$this->bbURL."/tde_busca/arquivo.php?codArquivo=".$cod));
	}
	...
	function getArquivo($id=null)
	{
		...
		if ($id!=null){
			...
			// Patch de segurança
			// 10/03/2006 - Gabriel
	      		// ANTES : $t=$this->tag_obrigatorio('URL',str_replace("http://http://","http://",$this->trata("http://".$this->bbURL.str_replace("..","",$row['arURL']))),$a);
	...
	function getBibliotecaDigitalArquivo($id)
	{
		...
		while($row = @mysql_fetch_array($query)){
			...
			while($row2=@mysql_fetch_array($query2)){
				...
				$l.=$this->tag_facultativo('Legenda',$this->trata($row2['laLegenda']),$a);
				$a=null;
			}
			$r.=$l;
	...
	function getBibliotecaDepositaria($tese)
	{
		if (is_array($tese)){
			$sql = "
				...
				AND Programa.prCod = Programa.prCod
	...
	{
		//$query=@mysql_query("SET NAMES utf8") || die("aa");
		...
		while ($row = @mysql_fetch_array($query)){
			...
			//$r = mb_convert_encoding($r,"UTF-8","iso-8859-1");
		}
		return $r;
	}
	...
	function getAssunto($id)
	{
		...
		while ($row = @mysql_fetch_array($query)){
			...
			$r.=$this->tag_facultativo('Assunto',$this->trata($row['asAssunto']),$att);
		}
		return $r;
	}
	...
	function getLocalDefesa($row)
	{
		if (is_array($row)){
			...
			$r.=$this->tag_facultativo('UF',$this->checkUF($row['tsUFLocalDefesa']));
	...
	function getInstituicaoDefesa($id_tese,$id_instituicao)
	{
		...
		if ($row = @mysql_fetch_array($query)){
			...
			$r=$this->tag_facultativo('InstituicaoDefesa',$this->tag_PessoaJuridica($row).$s);
		}
		return $r;
	}
	...
	function loadETDMS($id)
	{
		...
		if($rowTese=mysql_fetch_array(@mysql_query($sql))){
			...
			// TYPE ========================================
			if ($this->getArquivo($id)!=null) {
                	   $reg.= $this->tag_obrigatorio('type',"Eletronic Thesis or Dissertation");
                	   $reg.= $this->tag_obrigatorio('type',"Tese ou Dissertação Eletrônica");
			} else {
			   $reg.= $this->tag_obrigatorio('type',"Printed Thesis or Dissertation");
			   $reg.= $this->tag_obrigatorio('type',"Tese ou Dissertação Impressa");
			}
			...
			while ($row = @mysql_fetch_array($query)){
			   //$a['Idioma'] = $this->checkIdioma($row['drIdioma']);
			   $reg.=$this->tag_facultativo('rights',$this->trata($row['drDireito']));
			}
			...
			$query=@mysql_query($sql);
			if ($row = @mysql_fetch_array($query)){
			   $regDEG.=$this->tag_facultativo('grantor',$this->trata($row['Nome']));
			}
	...
	function load($id)
	{
		if($this->_version=="oai_dc") return $this->loadOAIDC($id);
		if($this->_version=="oai_etdms") return $this->loadETDMS($id);
		...
		if($row=mysql_fetch_array(@mysql_query($sql))){
			...
			$reg.=$this->tag_facultativo('Titulacao',$row['tsTitulacao']);
			...
			if ($this->_version == "2.0"){
				$reg.=$this->getInstituicaoDefesa($row['tsIdentificador'],$row['inCod']);
			}else{
	...
	function show()
	{
		...
		if ($this->checkDeleted()){
			return null;
		}elseif ($this->_version == "oai_etdms"){
			$a['xmlns:oai_etdms']="http://www.ndltd.org/standards/metadata/etdms/1.0/";
			$a['xmlns:xsi']="http://www.w3.org/2001/XMLSchema-instance";
			$a['xsi:schemaLocation']="http://www.ndltd.org/standards/metadata/etdms/1.0/
...
class OAI_MTDBR extends TEDE_MTDBR{
	...
	function listIdentifiers($maxidentifiers, $from = null, $until = null, $oaiset = null, $offset = 0)
	{
		...
		if ($offset > 0)$sql.= " LIMIT $offset,99999999 ";
	...
	// PUBLIC 
	function listRecords($maxrecords, $from = null, $until = null, $oaiset = null, $offset = 0,$record_prefix=null)
	{
		global $repositoryFilter;		
		$resp=null;
