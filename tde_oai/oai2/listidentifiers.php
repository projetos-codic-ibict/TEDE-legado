...
if (!isset($errors)) {
	$list = $TEDE->listIdentifiers($MAXIDS,$from,$until,$set,$deliveredrecords);
	if (count($list['err'])>0) {
		$errors .= oai_error('noIdentifiersMatch'); 
	}
        if (!isset($list['verb'])) {
                $errors .= oai_error('noRecordsMatch');
        }
...
elseif (isset($args['resumptionToken'])) {
   $restoken = '  <resumptionToken completeListSize="'.$list['count'].'"
