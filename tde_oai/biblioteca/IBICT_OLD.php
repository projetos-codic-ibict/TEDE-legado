...
function trata($metadata,$is_amiguinho = false){
	...
	$metadata=str_replace(chr(194).chr(130),'\'',$metadata);           //
	$metadata=str_replace(chr(194).chr(132),'"',$metadata);           //
	$metadata=str_replace(chr(194).chr(139),'\'',$metadata);  //
	$metadata=str_replace(chr(194).chr(145),'\'',$metadata);           //
	$metadata=str_replace(chr(194).chr(146),'\'',$metadata);           //
	$metadata=str_replace(chr(194).chr(147),'"',$metadata);           //
	$metadata=str_replace(chr(194).chr(148),'"',$metadata);           //
	$metadata=utf8_decode($metadata);

	return htmlspecialchars($metadata);
	return $metadata;
}
...
//				$this->erro('Idioma utilizado fora do padrão de 3 letras '.$id);
	...
	{
		return $this->tag_obrigatorio('Tipo','Eletronic Thesis or Dissertation');
	}
    ...
    while($row = @mysql_fetch_array($query)){
      ...
      $r.=$this->tag_obrigatorio('URLDocumento',$this->bbURL.str_replace("..","",$row['arURL']),$a);
    }
    return $r;
	...
	function getPessoaFisica($tag,$id)	
	{
		...
		while($row = @mysql_fetch_array($query)){
			...
			$r.=$this->tag_facultativo('Lattes',$row['pfLattes']);
	...
	function getPrograma($id,$inId=1)
	{
		$sql = "
			SELECT * 
			FROM Programa 
			INNER JOIN TSPF 
				ON (Programa.prCod = TSPF.prCod) WHERE TSPF.tsIdentificador = \"$id\" ";
		$query=@mysql_query($sql);
		$r=null;
		if($row = @mysql_fetch_array($query)){
			...
			$i=$this->getProgramaInstituicao($inId);
	...
	function getProgramaInstituicao($id)
	{
		...
		$resp=null;
		if ($row = @mysql_fetch_array($query)){
			$resp = $this->tag_facultativo('Instituicao',$this->tag_PessoaJuridica($row));
		}
		return $resp;
	}
...
class OAI_MTDBR extends TEDE_MTDBR{
	...
	function listIdentifiers($maxidentifiers, $from = null, $until = null, $oaiset = null, $offset = 0)
	{
		...
		$sql.= ' FROM_UNIXTIME(UNIX_TIMESTAMP(tsDataAtualizacao)+3*3600) as datestamp ';
		$sql.= ' FROM Teses ';
		$sql.= ' WHERE tsIdentificador = tsIdentificador ';
		...
		while($row = mysql_fetch_array($query)){
			if ($i < $maxidentifiers){
				...
				$t['continue'].=$this->tag_header($row);
			}
			$i++;
		}
		if ($i==0){
		...
		if ($query = mysql_query($sql)){
			...
			while($row = mysql_fetch_array($query)){
				if ($i < $maxrecords){
					...
					$t['continue'].=$this->tag_record($row);
				}
				$i++;
			}
		}else{
