...
class TEDE_MTDBR extends TAGXML
{
	...
	* @return object TEDE_MTDBR
		...
		if ($id!=null){
			...
			// 10/03/2006 - Gabriel
	      		// ANTES : $t=$this->tag_obrigatorio('URL',str_replace("http://http://","http://",$this->trata("http://".$this->bbURL.str_replace("..","",$row['arURL']))),$a);
	      		 $t=$this->tag_obrigatorio('URL',str_replace("http://http://","http://",$this->trata("http://".$this->bbURL."/tde_busca/arquivo.php?codArquivo=".$row['arCod'])),$a);
	...
	function load($id)
	{
		if($this->_version=="oaidc") return $this->loadOAIDC($id);
	...
	function show()
	{
		...
		if ($this->checkDeleted()){
			return null;
		}elseif ($this->_version == "oaidc"){
			$a['xmlns']="http://www.openarchives.org/OAI/2.0/oai_dc/";
