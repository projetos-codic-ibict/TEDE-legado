	...
	require "../conexao/conexao_fim.inc";
	 if ($sucesso!="" or $erro!="") { ?>
			...
			<table width="90%" border="0" align="center" cellpadding="5" cellspacing="0" class="loginTable">
				...
				<td colspan="2" class="txtBold"><? echo "$txInstituicaoDefesa"; ?></td>
                </tr>
                        <tr class="txt"> 
				...
				$AinSigla=$SEL_in_RES["inSigla"];
				$AinPais=$SEL_in_RES["inPais"];
				$AinUF=$SEL_in_RES["inUF"];
				$inCNPJ=$SEL_in_RES["inCNPJ"];
				$AinURL=$SEL_in_RES["inURL"];
				?>
                            <? echo $txNome; ?></td>
				 <td width="91%"> 
				 <input name="inNome" type="text" class="req" value="<? if ($inNome=="") {echo $AinNome; }else{echo $inNome;} ?>" size="50" maxlength="100">
				  </td>
				 </tr>
				 <tr class="txt"> 
				 <td height="20" class="txtBold"><? echo $txSigla; ?></td>
				 <td> <input name="inSigla" type="text" class="input" value="<? if ($inSigla=="") {echo $AinSigla; }else{echo $inSigla;} ?>" size="25" maxlength="30"> 
				 </td>
				 </tr>
				 <tr class="txt"> 
				 <td class="txtBold"><? echo $txPais; ?></td>
				          <td><select name="inPais" class="req" onChange="submit()">
                              <?
							require "../conexao/conexao_fim.inc";
							$SEL_psin="SELECT inPais FROM Instituicao WHERE inCod='1'";
							...
							?>
                              <option value="<? echo $psSigla; ?>" <? if ($psSigla==$AinPais and   $inPais=="") { echo "selected"; } else { if($psSigla==$inPais) { echo "Selected"; }} ?>><? echo $psNome; ?></option>
                              <?
						   $SEL_ps_RES=mysql_fetch_array($SEL_ps_query);
						   }
						   ?>
				 ...
				 <tr class="txt"> 
				          ...
				          <td>
                            <?
							if ($inPais=="") {
						$SEL_ps="SELECT * FROM Pais WHERE psIdiomaPais='pt-BR' and psSigla='$AinPais'"; } else { 
						...
						?>
							...
							?>
                              <option value="<? echo $ufSigla; ?>" <? if ($ufSigla==$AinUF) { echo "selected"; } ?>><? echo $ufNome; ?></option>
                              <?
						   $SEL_uf_RES=mysql_fetch_array($SEL_uf_query);
						   }
						   ?>
				 ...
				 <td> <input name="inURL" type="text" class="input" id="inURL" value="<? if ($inURL=="") {echo $AinURL; }else{echo $inURL;} ?>" size="40" maxlength="150"> 
				 </td>
				 </tr>
				 </table></td>
				 </tr>
				 <tr> 
				 <td align="right" valign="bottom"><input name="pjCod" type="hidden" class="input" id="pjCod" value="<? echo $inCod;?>">
